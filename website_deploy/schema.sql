...
-- Typing Mastery Classes table (added 2026-03-05)
CREATE TABLE IF NOT EXISTS typing_classes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    level TEXT NOT NULL,
...
-- Typing Mastery Progress table
CREATE TABLE IF NOT EXISTS typing_progress (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL REFERENCES users(id),
    class_id INTEGER NOT NULL REFERENCES typing_classes(id),
    completed INTEGER DEFAULT 0,
    completed_at TEXT,
    wpm_score INTEGER DEFAULT 0,
    accuracy_score REAL DEFAULT 0,
    UNIQUE(user_id, class_id)
);

-- Typing Mastery Indexes
CREATE INDEX IF NOT EXISTS idx_typing_classes_level_sort ON typing_classes(level, sort_order);
CREATE INDEX IF NOT EXISTS idx_typing_progress_user ON typing_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_typing_progress_class ON typing_progress(class_id);
